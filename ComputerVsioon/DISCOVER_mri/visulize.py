...
def tsne_latent_plot(encoder, dataloader, device, n_batches=5):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_latent_tsne_v1.png")
    plt.show()
...
def plot_binary_classification_scores(img_clf, latent_clf, encoder, dataloader, device):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_correlation_v1.png")
    plt.show()
...
def compute_latent_class_correlations(encoder, clf_model, dataloader, latent_dim, device):
    ...
    with torch.no_grad():
        for imgs, _ in dataloader:
            imgs = imgs.to(device)
            z, _, _ = encoder(imgs)  # shape: [B, latent_dim]
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_pearson_v1.png")
    plt.show()
...
def visualize_counterfactual_changes(img, encoder, decoder, stds, selected_features, std_range=3.0):
    ...
    with torch.no_grad():
        z_orig, _, _ = encoder(img.unsqueeze(0))
        z_orig = z_orig.squeeze(0)

        img_orig = decoder(z_orig.unsqueeze(0)).squeeze().cpu().numpy()
        ...
        for feat in selected_features:
            ...
            img_plus = decoder(z_plus.unsqueeze(0)).squeeze().cpu().numpy()
            img_minus = decoder(z_minus.unsqueeze(0)).squeeze().cpu().numpy()
        ...
        axs[2, 0].set_ylabel("Z+ Diff", rotation=0, labelpad=20)
        axs[3, 0].set_ylabel("Z-", rotation=0, labelpad=20)
        axs[4, 0].set_ylabel("Z- Diff", rotation=0, labelpad=20)

        plt.tight_layout()
        plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_counter_factual_v1.png")
        plt.show()
...
def visualize_multiple_latent_traversals(encod, decod, img, z_std, selected_features, steps=7, std_range=3, device='cuda'):
    ...
    with torch.no_grad():
        z_orig, _, _ = encod(img)  # shape: [1, latent_dim]
    ...
    plt.tight_layout()
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_latent_traversal_v1.png")
    plt.show()
...
def sample_and_decode(decoder, latent_dim=150, num_samples=25, device='cuda'):
    ...
    with torch.no_grad():
        ...
        plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_sampled_realistic_images_v1.png")
        plt.show()
...
def visualize_reconstruction(encod, decod, dataloader, device='cuda', num_images=8):
    ...
    with torch.no_grad():
        ...
        print(recon[:,0].unsqueeze(1).shape)
        print(imgs[:,0].unsqueeze(1).shape)
        # Concatenate original and reconstructed images for visualization
        comparison = torch.cat([imgs[:,0].unsqueeze(1), recon[:,0].unsqueeze(1)], dim=0)  # 2N images
        ...
        plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_recontruction_ability_v1.png")
        plt.show()

def subset_clf_performace(model_subset, encoder, data_loader, device):
    all_labels = []
    all_probs = []
    with torch.no_grad():
        CM = 0
        ...
        for imgs, labels in data_loader:
            ...
            z_img, _, _ = encoder(imgs)
            preds = torch.squeeze(model_subset(z_img[:, :14]))
            preds_bool = preds >= 0
            labels = labels.float()
    ...
    plt.grid(True)
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/kld_test_subset_metrics.png")
    tn=CM[0][0]
    tp=CM[1][1]
    fp=CM[0][1]
    fn=CM[1][0]
...
def main():
    ...
    # test_loader =  torch.utils.data.ConcatDataset([data_dead_test, data_alive_test])
    encod = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/models/encod_kld_version_train_1_epoch54_saved.pt", map_location = device, weights_only = False).eval()
    decod = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/models/decod_kld_version_train_1_epoch54_saved.pt", map_location = device, weights_only = False).eval()
    img_clf_model = torch.load("/user/sina.garazhian/u12203/DISCOWER/best_vgg_1.pt", weights_only = False, map_location=device).eval()
    # img_clf_model = torch.load("/user/sina.garazhian/u12203/panc_cell/best_clf.pt", weights_only = False, map_location=device).eval()
    
    latent_clf_model = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER_mri/models/subset_kld_version_train_1_epoch54_saved.pt", map_location = device, weights_only = False).eval()
    ...
    # pearsons_corr = compute_latent_class_correlations(encod, img_clf_model, test_loader, 350, device)
    ...
    # visualize_counterfactual_changes(test_img_normal_dataset[80][0], encod, decod, stds, np.argsort(np.abs(pearsons_corr))[-4:], std_range=3.0)
    # visualize_multiple_latent_traversals(encod, decod, test_img_normal_dataset[80][0][np.newaxis, :, :, :], stds, np.argsort(np.abs(pearsons_corr))[-8:], steps=7, std_range=3, device='cuda')
    subset_clf_performace(latent_clf_model, encod, test_loader, device)
    # sample_and_decode(decod, latent_dim=350, num_samples=25, device='cuda')
