...
def tsne_latent_plot(encoder, dataloader, device, n_batches=5):
    ...
    with torch.no_grad():
        for i, (imgs, _, _, labels) in enumerate(dataloader):
            # if i >= n_batches:
            #     break
            imgs = imgs.to(device)
            z = encoder(imgs).cpu()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER/kdl_all_losses_test_latent_tsne_v1.png")
    plt.show()
