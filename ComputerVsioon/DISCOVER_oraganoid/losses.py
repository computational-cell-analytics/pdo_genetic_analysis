...
class PerceptionLossVGG_hook(nn.Module):
    def __init__(self, device):
        super().__init__()
        # Load full VGG19 (features + classifier)
        self.vgg = vgg19(weights=VGG19_Weights.IMAGENET1K_V1).to(device).eval()

        for param in self.vgg.parameters():
            param.requires_grad = False

        # Register hook layer names (exact names in torchvision VGG)
        self.selected_layers = {
            'features.10': 1.0,  # block3_conv1
            'features.12': 1.0,  # block3_conv2
            'features.14': 1.0,  # block3_conv3
            'features.19': 1.0,  # block4_conv1
            'features.21': 1.0,  # block4_conv2
            'features.23': 1.0,  # block4_conv3
            'features.25': 1.0,  # block4_conv4
            'features.28': 1.0,  # block5_conv1
            'features.30': 1.0,  # block5_conv2
            'features.32': 1.0,  # block5_conv3
            'features.34': 1.0,  # block5_conv4
        }
    ...
    def _register_hooks(self):
        ...
        for name, module in self.vgg.named_modules():
            if name in self.selected_layers:
                def make_hook(n):
                    return lambda m, inp, out: self._hook_dict.setdefault(n, out)
    ...
    def forward(self, output, target):
        ...
        _ = self.vgg(output)
        features_x = self._hook_dict.copy()

        self._hook_dict = {}
        self._register_hooks()
        _ = self.vgg(target)
        features_y = self._hook_dict.copy()
        ...
        for name in self.selected_layers:
            fx = features_x.get(name)
            fy = features_y.get(name)
            if fx is None or fy is None:
                raise ValueError(f"Missing activation for layer {name}")
            loss += self.selected_layers[name] * self.mse(fx, fy)
...
def Discriminator_loss(real, fake):
    ...
    real_loss = F.binary_cross_entropy_with_logits(real, real_labels)
    fake_loss = F.binary_cross_entropy_with_logits(fake, fake_labels)
...
class FeatureExtractor_CellImageEncoder(nn.Module):
    def __init__(self, model, target_layers):
        super(FeatureExtractor_CellImageEncoder, self).__init__()
        self.model = model
        self.model.eval()
        self.target_layers = target_layers  # e.g., ['conv1', 'res_block', 'fusion']
        self.features = {}
    ...
    def forward(self, x):
        ...
        return self.features  # dict of {layer_name: feature_map}
