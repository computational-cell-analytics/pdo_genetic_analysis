...
def visualize_counterfactual_changes(img, encoder, decoder, stds, selected_features, std_range=3.0):
    ...
    with torch.no_grad():
        ...
        for i in range(len(selected_features)):
            ...
            axs[2, i].imshow(plus_diffs[i], cmap='gray'); axs[3, i].set_title("Diff(+)"); axs[3, i].axis("off")
        ...
        plt.tight_layout()
        plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER/models/mmd_panc_test_counter_factual_v1_edited.png")
        plt.show()
...
def main():
    ...
    encod = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER/models/encod_mmd_version_train_2.pt", map_location = device, weights_only = False).eval()
    decod = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER/models/decod_mmd_version_train_2.pt", map_location = device, weights_only = False).eval()
    ...
    latent_clf_model = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/DISCOWER/models/subset_mmd_version_train_2.pt", map_location = device, weights_only = False).eval()
