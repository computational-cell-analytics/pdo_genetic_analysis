...
sampler = dataset.BalancedBatchSampler(labels, patients_per_batch=8, samples_per_patient=4)
...
def train_model(model, train_loader, device='cuda', epochs=50, patients_per_batch=8, samples_per_patient=4, lr=1e-3):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 2 == 0:
            ...
            if patience_counter >= 2:
                ...
                model_path = f"/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/checkpoints_84/model_epoch{epoch+1}_sep{current_sep_score:.4f}.pt"
                torch.save(model.state_dict(), model_path)
                sep_checkpoints.append(model_path)
                print(f"✅ Saved model with separability {current_sep_score:.4f} at epoch {epoch+1}")
                features, labels = helper_function.extract_flat_features(model)
                print("getting featurs done")
                helper_function.tsne_and_plot(features, labels, f"{epoch + 1}_84")
...
# torch.save(model_trained, "/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/checkpoints_84/represent_model_epoch50.pt")
...
encoder.load_state_dict(torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/checkpoints_84/model_epoch30_sep2.0956.pt"))

model_clf = train_classifier(model_clf, encoder, train_loader, val_loader)
# torch.save(model_clf, "/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/clf_model_epoch20.pt")
# model_clf = torch.load("/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/clf_model_epoch20.pt", weights_only=False)
...
def evaluate_classifier(encoder, model_clf, test_loader, class_names=None, device='cuda', plot_cm=True):
    ...
    if plot_cm:
        ...
        plt.tight_layout()
        plt.savefig("/user/sina.garazhian/u12203/lustere-grete-mine/representaion_learning/confusion_mat_epoch18_84_20.png")
        # plt.show()
