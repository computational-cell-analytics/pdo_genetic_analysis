    ...
    def __init__(self, filtered_csv, width_threshold, heigth_threshold, cell_label, transform=None):
        ...
        for index, row in self.filetered_metadata.iterrows():
            ...
            try:
                    ...
                    self.file_names += [file_name] * len(cells)
...
class FullEpochBalancedSampler(Sampler):
    """
    Ensures each class (e.g., patient) is equally represented in each epoch.
    Smaller classes are sampled with replacement, larger ones without.
    """
    def __init__(self, dataset):
        """
        Args:
            dataset: your PyTorch dataset
            get_label_fn: a function to extract the label (e.g., patient ID) from a dataset item
        """
        ...
        self.max_len = max(len(indices) for indices in self.label_to_indices.values())
    ...
    def __iter__(self):
        ...
        for label in self.labels:
            ...
            if len(indices) < self.max_len:
                sampled = random.choices(indices, k=self.max_len)  # with replacement
            else:
                sampled = random.sample(indices, k=self.max_len)  # without replacement
    ...
    def __len__(self):
        return self.max_len * len(self.labels)
